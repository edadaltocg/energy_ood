...
parser.add_argument(
    ...
    "-m",
    type=str,
    default="cifar10_allconv_baseline",
    help="Method name.",
)
# Loading details
...
parser.add_argument(
    "--out_as_pos", action="store_true", help="OE define OOD data as positive."
)
parser.add_argument(
    "--score", default="MSP", type=str, help="score options: MSP|energy"
)
parser.add_argument("--T", default=1.0, type=float, help="temperature: energy|Odin")
...
ood_loader = torch.utils.data.DataLoader(
    ood_data, batch_size=args.test_bs, shuffle=True, num_workers=4, pin_memory=True
)
print("\n\nTexture Detection")
...
ood_loader = torch.utils.data.DataLoader(
    ood_data, batch_size=args.test_bs, shuffle=True, num_workers=1, pin_memory=True
)
print("\n\nLSUN_C Detection")
get_and_print_results(ood_loader)
...
ood_data = torch.from_numpy(
    np.random.uniform(
        size=(ood_num_examples * args.num_to_avg, 3, 32, 32), low=-1.0, high=1.0
    ).astype(np.float32)
)
    ...
    def __getitem__(self, i):
        ...
        return (
            trn.Normalize(mean, std)(
                torch.sqrt(self.dataset[i][0] * self.dataset[random_idx][0])
            ),
            0,
        )
...
jigsaw = lambda x: torch.cat(
    (
        ...
        torch.cat(
            (x[:, 16:, 16:], torch.cat((x[:, :16, 24:], x[:, :16, 16:24]), 2)), 2
        ),
    ),
    1,
)
...
pixelate = lambda x: x.resize((int(32 * 0.2), int(32 * 0.2)), PILImage.BOX).resize(
    (32, 32), PILImage.BOX
)
ood_loader.dataset.transform = trn.Compose(
...
rgb_shift = lambda x: torch.cat(
    (
        x[1:2].index_select(2, torch.LongTensor([i for i in range(32 - 1, -1, -1)])),
        x[2:, :, :],
        x[0:1, :, :],
    ),
    0,
