...
def print_measures_with_std(rmss, mads, sf1s, method_name="Baseline"):
    print("\t\t\t\t\t\t\t" + method_name)
    print(
        "RMS Calib Error (%): \t\t{:.2f}\t+/- {:.2f}".format(
            100 * np.mean(rmss), 100 * np.std(rmss)
        )
    )
    print(
        "MAD Calib Error (%): \t\t{:.2f}\t+/- {:.2f}".format(
            100 * np.mean(mads), 100 * np.std(mads)
        )
    )
    print(
        "Soft F1 Score (%):   \t\t{:.2f}\t+/- {:.2f}".format(
            100 * np.mean(sf1s), 100 * np.std(sf1s)
        )
    )
...
def show_calibration_results(confidence, correct, method_name="Baseline"):
    ...
    print("Soft F1-Score (%): \t\t{:.2f}".format(100 * soft_f1(confidence, correct)))
